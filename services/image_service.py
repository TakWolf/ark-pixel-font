...
def make_itch_io_cover():
    ...
    _draw_text(image, (image.width / 2, 6 + line_height * 2), '我们每天度过的称之为日常的生活，\n其实是一个个奇迹的连续也说不定。', font_zh_cn, text_color=text_color, shadow_color=shadow_color, is_horizontal_centered=True)
    _draw_text(image, (image.width / 2, 6 + line_height * 4), '我們每天度過的稱之為日常的生活，\n其實是一個個奇跡的連續也說不定。', font_zh_tr, text_color=text_color, shadow_color=shadow_color, is_horizontal_centered=True)
    _draw_text(image, (image.width / 2, 6 + line_height * 6), '日々、私たちが過ごしている日常は、\n 実は奇跡の連続なのかもしれない。', font_ja, text_color=text_color, shadow_color=shadow_color, is_horizontal_centered=True)
    ...
    _draw_text(image, (image.width / 2, 6 + line_height * 11), '★☆☺☹♠♡♢♣♤♥♦♧\n☀☼♩♪♫♬☂☁⚓✈⚔☯', font_latin, text_color=text_color, shadow_color=shadow_color, is_horizontal_centered=True)
    image = image.resize((image.width * 2, image.height * 2), Image.NEAREST)
