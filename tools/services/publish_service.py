...
def make_release_zips(font_size: FontSize, width_mode: WidthMode, font_formats: Sequence[FontFormat]) -> None:
    ...
    for font_format in font_formats:
        zip_file_path = path_define.RELEASES_DIR.joinpath(f'ark-pixel-font-{font_size}px-{width_mode}-{font_format}-v{configs.VERSION}.zip')
        with ZipFile(zip_file_path, 'w') as file:
            ...
            for language_flavor in options.LANGUAGE_FLAVORS:
                font_file_path = path_define.OUTPUTS_DIR.joinpath(f'ark-pixel-{font_size}px-{width_mode}-{language_flavor}.{font_format}')
